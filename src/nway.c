                            ...
                            struct mslab *int_64_t_slab)
{
    ...
    */
    ...
    // Create nways from the orders before s_i
    int i;
#ifdef GET_NWAY_SWEEP_LIST_MEM_TIME
    struct timeval __t1 = in();
    unsigned long t1a = 0, t1b = 0, t1c = 0, t1d = 10000000;
#endif

    for (i = 0; i < num_sets; i++) {
        if (i == s_i) {
            struct timeval __t1a = in();
            // Add the new item to the list
            struct int_list_list *curr = head;
            while (curr != NULL) {
#ifdef GET_NWAY_SWEEP_LIST_MEM_TIME
                struct timeval __t1c = in();
#endif
                curr->list[s_i] = ordering[s_i].end + 1;
#ifdef GET_NWAY_SWEEP_LIST_MEM_TIME
                unsigned long ___t1c = out(__t1c);
                t1c = MAX(t1c,___t1c);
#endif
                curr = curr->next;
            }
#ifdef GET_NWAY_SWEEP_LIST_MEM_TIME
            t1a = MAX(t1a,out(__t1a));
#endif
        } else { 
            // set the first item in the ordering
...
#ifdef GET_NWAY_SWEEP_LIST_MEM_TIME
                struct timeval __t1b = in();
#endif
                curr->list[i] = ordering[i].start;
#ifdef GET_NWAY_SWEEP_LIST_MEM_TIME
                unsigned long ___t1b = out(__t1b);
                t1b = MAX(t1b,___t1b);
                t1d = MIN(t1d,___t1b);
#endif
                curr = curr->next;
                      ...
                      pri_queue c)
{
    ...
    *R_tail = nways_tail;
    ...
    // clean out pqs
    int64_t tmp;
    while ( priq_top(c, &tmp) != NULL ) {
        int *t2 = priq_pop(c, &tmp);
    }

    while ( priq_top(q, &tmp) != NULL ) {
        int *t2 = priq_pop(q, &tmp);
    }
}
//}}}
...
void sweep_split_subset(struct interval **S,
                        ...
                        int *num_R)
{
    struct int_list_list *nways_head = NULL, *nways_tail = NULL;
    int i, total_intervals = 0;

    for(i=0; i < num_sets; i++)
        total_intervals += s_dim[i].end - s_dim[i].start + 1; 

    if (total_intervals == num_sets) {
        struct int_list_list *new_l = (struct int_list_list *)
                malloc(sizeof(struct int_list_list));
        new_l->next = NULL;
        new_l->size = num_sets;
        new_l->list = (int64_t *)
                malloc(sizeof(int64_t)*num_sets);

        int64_t start_v=0, end_v=LLONG_MAX;
                 ...
                 struct int_list_list **R)
{
    ...
    /*
    struct timeval __n = in();
    struct pair **centers = (struct pair **)
            malloc(set_sizes[0]*sizeof(struct pair *));
    int i;
    for (i = 0; i < set_sizes[0]; ++i)
       centers[i] = NULL; 
    l1_split_sets_o_centers(S,
                            set_sizes,
                            centers,
                            num_sets);
    unsigned long n = out(__n);
    printf("%lu\t%lu\n", o, n);
    */
    ...
    struct mslab *int_64_t_slab = 
            mslab_init(sizeof(int64_t) * num_sets, 1000);
    ...
    */
    ...
    /*
    struct mslab *list_list_slab = mslab_init(sizeof(struct int_list_list), 
            set_sizes[0]*2);
    struct mslab *list_slab =  mslab_init(sizeof(int64_t)*num_sets,
            set_sizes[0]*2);
    */

    /*
    struct int_list_list list_list[set_sizes[0]*2];
    int64_t list[set_sizes[0]*2*num_sets];
    */
    /*
    struct int_list_list *list_list = (struct int_list_list *)
            malloc(set_sizes[0]*2 * sizeof(struct int_list_list));
    int64_t *list = (int64_t *)
            malloc(set_sizes[0]*2*num_sets * sizeof(int64_t));
    */

    struct int_list_list *R_head=NULL, *R_tail=NULL;
    int i,total_intervals;
    while (curr != NULL) {
...
//{{{
        ...
        total_intervals=0;
        for(i=0; i < num_sets; i++)
            total_intervals += node->s_dim[i].end - node->s_dim[i].start + 1; 
        ...
        if (total_intervals == num_sets) {
            ...
            int64_t start_v=0, end_v=LLONG_MAX;
            for(i=0; i < num_sets; i++) {
                start_v = MAX(start_v, S[i][node->s_dim[i].start].start);
                end_v = MIN(start_v, S[i][node->s_dim[i].start].end);
                new_l->list[i] = node->s_dim[i].start;
            }
            if (end_v >= start_v) {
                if (R_head == NULL) 
                    R_head = new_l;
...
void *run_sweep_subset(void *arg)
{
    ...
    pri_queue q = priq_new(p->num_sets * 10);
    pri_queue c = priq_new(p->num_sets * 10);
...
#ifdef RUN_GET_CENTER_SPLIT_TIME
        struct timeval _t = in();
#endif
                              ...
                              int num_sets)
{
    ...
    while (curr != NULL) {
        ...
        if (center != NULL) 
            centers[curr->s_dim[0].start] = center->s_dim;
                 ...
                 int num_sets)
{
    ...
    *leaf_head = NULL;
    ...
    while (curr != NULL) {
...
#ifdef SPLIT_SIZES
        int i;
        int64_t min_sdim = LLONG_MAX, max_sdim = -1;
        int64_t min_l = LLONG_MAX, max_l = -1;
        double min_d = (double) LLONG_MAX, max_d = -1;
        for (i = 0; i < num_sets; ++i) {
            min_sdim = MIN(min_sdim, 
                    curr->s_dim[i].end - curr->s_dim[i].start + 1);
            max_sdim = MAX(max_sdim, 
                    curr->s_dim[i].end - curr->s_dim[i].start + 1);

            min_l = MIN(min_l, 
                        S[i][curr->s_dim[i].end].end - 
                        S[i][curr->s_dim[i].start].start);
            max_l = MAX(max_l, 
                        S[i][curr->s_dim[i].end].end - 
                        S[i][curr->s_dim[i].start].start);

            min_d = MIN(min_d, 
                        ( (double)(curr->s_dim[i].end - 
                                   curr->s_dim[i].start + 1)) /
                        ( (double) (S[i][curr->s_dim[i].end].end - 
                                    S[i][curr->s_dim[i].start].start)));
            max_d = MAX(max_d, 
                        ( (double)(curr->s_dim[i].end - 
                                   curr->s_dim[i].start + 1)) /
        ...
        printf("%llu\tmin_n:%llu\tmax_n:%llu\t"
               "min_l:%llu\tmax_l:%llu\tmin_d:%f\tmax_d:%f\n",
               curr->S_dim.start,min_sdim, max_sdim, min_l, max_l,
               min_d,max_d);
#endif
                ...
                int *num_splits)
{
    ...
    *num_threads = 1;
    ...
    while ( (c = getopt(argc, argv, "f:n:i:s:l:p:r:t:I:S:") ) != -1) 
        switch(c) {
            case 'f':
                file_name = optarg;
                break;
